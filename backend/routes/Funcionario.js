  ...
  // ── LISTAR VENDEDORES — para o formulário de venda (vendedor pode acessar) ──
router.get('/vendedores', auth, permissao(1, 2, 3), async (req, res) => {
  try {
    res.json(await q(
      `SELECT id_funcionario, nome FROM funcionario WHERE cargo = 3 AND ativo = 1 ORDER BY nome`
    ));
